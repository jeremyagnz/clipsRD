...
test('ScriptGenerationService repara hooks inválidos y guarda el guion final', async () => {
  ...
  try {
    ...
    const scriptPayload = JSON.stringify({
      ...
      script: 'Hay lugares del océano más desconocidos que la Luna. Y no es exageración. Apenas exploramos una parte mínima del fondo marino. Cada inmersión profunda revela especies extrañas, paisajes imposibles y señales que todavía intentamos entender. Lo más fascinante es que muchas respuestas siguen allá abajo. Por eso cada descubrimiento cambia lo que creíamos saber del planeta. Si te sorprendió, sigue la cuenta para más historias reales del océano.',
